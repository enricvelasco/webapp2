...
var esUpdate = false
export class ListadoCategoriasProducto extends Component{
 constructor(props, context) {
   ...
   this._cargarDatos()
   console.log("CONSTRUCTOS CATEGoRIAS LOCAL");
 }
 ...
 _cargarDatos(){
   ...
   let rows = [];
   db.collection("categoriasProducto").get().then((querySnapshot) => {
 ...
 _eliminarCampos =(e)=>{
   ...
   this.setState({
     ...
     db.collection("categoriasProducto").doc(this.state.camposGridSeleccionados[0].codigoLink.id).delete()
     .then(() => {
         ...
         <div className="box margenes-box-listado">
           <div className="columns">
             <div className="column">
               TITULO CATEGORIAS LOCAL
             </div>
           </div>
           <div className="columns">
               <div>
                 <Link to='/categoriasProducto/new' className="button is-primary" onClick={((e) => this._cargarFormulario(e, 'nuevo', null))}>Nuevo</Link>
               </div>
               <div>
