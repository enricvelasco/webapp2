...
export class FormularioCategoriasProducto extends Component{
	constructor(props) {
    super(props);
	...
	_saveFormulario=(e)=>{
		...
		if(this.props.parametros == null){//hace un nuevo
			db.collection("categoriasProducto").add(this.state.objGuardar)
			...
			db.collection("categoriasProducto").doc(this.props.parametros.id).set(this.state.objGuardar, { merge: true })
			.then(()=> {
					this.props.onResults("listado")
    ...
    return(
	        <div className="box margenes-box-listado">
						<ul>
							...
							<li><Select onResults={this._retornoCampo} obligatorio={false} tituloCampo={"Categoría Padre"} campo={"idCategoriaProductoPadre"} url={"categoriasProducto"} filtro={["idCategoriaProductoPadre","==",null]} camposMostrar={["codigo", "nombre"]} valor={this.props.parametros != null? this.props.parametros.idCategoriaProductoPadre : null}/></li>
